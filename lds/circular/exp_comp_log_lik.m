...
T = length(data);
mu = interp( interp_coord( map, N ), params.template, 0 ); % Expected data
...
ecll = ( -extt(1)/2 ...
       ...
       + ( -mu*mu'/2 + mu*data' - data*data'/2 )/params.sig_temp^2 ...
       - (T-1)*log(params.sig_dt) - T*log(params.sig_temp) - (2*T-1)*log(2*pi)/2;