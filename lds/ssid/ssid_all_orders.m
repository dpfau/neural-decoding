function [moesp_params n4sid_params s] = ssid_all_orders( y, u, i, opts )
% Same as ssid.m, but evaluates system parameters for all orders from 1 up
% to opts.maxOrder.  Zero order model is just linear regression.
...
for n = 1:opts.maxOrder
    ...
    n4sid_params(n) = struct('A',A,'B',B,'C',C,'D',D,'Q',Q,'R',R,'S',S,'X',X);
    ...
    moesp_params(n) = struct('A',A,'B',B,'C',C,'D',D,'x0',x0);
end
