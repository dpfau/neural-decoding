...
CbD = newtons_method( @(x) data_ll( x, ...
                             data, ...
                             [map; ones(1,size(map,2));zeros(N*t,t), block_hankel(data,1,t,T-1)],...
                             covar.diag, ...
                             k), ...
                      [params.C, params.b, params.D], 1e-8, 1 );
...
H1 =  tprod( fCx, [1 -1], covar, [2 3 -1] ) ...
    + tprod( fCx, [1 -1], tprod( sigCt, [1 3 4], sigCt, [2 3 4] ), [2 3 1 -1] );
H2 = cat(3,cat(2,H1,zeros(x,y-dim,dim)),zeros(x,y,y-dim)) ...
   + tprod( fCx, [1 -1], tprod( map, [1 3], map, [2 3]), [2 3 -1] ) ...
   + tprod( fCx, [1 -1], tprod( map, [1 4], cat(1,sigCt,zeros(y-dim,x,z)), [2 3 4] ) ...
                       + tprod( cat(1,sigCt,zeros(y-dim,x,z)), [1 3 4], map, [2 4] ), [2 3 1 -1] );
Hinfo = struct('all',H2);
