...
ecll = 1/2*( trace( params.Q\Ptt - 2*(params.Q\A*Ptt1) + A'*(params.Q\A*Pt1t1) ) ) ...
    ...
    + 1/2*( trace( params.Q0\(covar.diag(:,:,1)+map(:,1)*map(:,1)') ) - 2*params.x0'*(params.Q0\map(:,1)) + map(:,1)'*(params.Q0\map(:,1)) )  ...
    + 1/2*log(det(params.Q0)) + numel(map)/2*log(2*pi) ...
    + sum( sum( exp( Cmu + 1/2*CsigCt ) - data.*Cmu + gammaln( data + 1 ) ) );