...
ll = 1/2*sum( sum( mapdiff.*(params.Q\mapdiff) ) ) + (size(map,2)-1)/2*log(det(params.Q))...
     + sum( sum( fCxb ) ) ...
     - sum( sum( data.*log( fCxb ) ) );

grad = [zeros(size(mapdiff,1),1), params.Q\mapdiff] ...
     - [params.A'*(params.Q\mapdiff), zeros(size(mapdiff,1),1)] ...
     + params.C'*fCxb1 - params.C'*(data.*fCxb1./fCxb);
 
Hinfo = struct('diag_upper',  params.A'*(params.Q\params.A), ...
